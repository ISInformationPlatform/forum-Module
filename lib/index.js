...
forum.updateComment = function(section_id, post_id, data, callback){
  var commentlist_section_collection = base_postcomment_collection + "_" + section_id;
  var postlist_section_collection = base_postlist_collection + "_" + section_id;
  var query = {
    "_id" : mongo.String2ObjectId(post_id)
  ...
  mongo.update(database, postlist_section_collection, query, update_list_obj, option, function(err, result){
    if(err) callback(err);
  });
  ...
  mongo.update(database, commentlist_section_collection, query, update_comment_obj, option, function(err, result){
    if(err) callback(err);
    console.log(result.result.n);
    callback(null, result.result.n);
  });
